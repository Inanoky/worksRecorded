...
export default function SiteDiaryCalendar({ siteId }) {
  ...
  const [currentMonth, setCurrentMonth] = React.useState(today.getMonth()); // 0-based: Jan=0
  ...
  return (
    <div className="w-full max-w-7xl mx-auto py-4">
      <h2 className="text-2xl font-semibold mb-2">Site Diary</h2>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-xl font-medium">{monthName} {currentYear}</div>
        <Button variant="outline" onClick={() => {
          if (currentMonth === 11) {
            setCurrentMonth(0);
            setCurrentYear(currentYear + 1);
        ...
        {weeks.map((week, i) =>
          week.map((date, j) => (
            <Card
              key={`${i}-${j}`}
              className={cn(
                "aspect-[1/0.7] min-h-[64px] flex items-center justify-center transition-all",
              ...
              <CardContent className="flex items-center justify-center p-0 h-full w-full">
                {date && (
                  <span className={cn(
                    ...
                    {date.getDate()}
                    {filledDays.includes(date.getDate()) && <span className="ml-1">✓</span>}
                  </span>
                )}
              </CardContent>
            </Card>
