...
export async  function systemPromptFunction(siteId, userId){
  ...
  `
    ...
    ` activities on site. If it is greeting, greet and adress him by his name. Your job extract all information you can gather from user message and save it calling the save_to_database tool once `
     +
    `siteId : ${siteId}
    userId : ${userId}
    Date today is : ${getTodayDDMMYYYY()} (format dd-mm-yyyy)
