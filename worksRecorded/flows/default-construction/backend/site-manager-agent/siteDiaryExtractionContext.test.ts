...
describe("buildSiteDiaryExtractionContext", () => {
  ...
  it("renders recent records without internal ids", async () => {
    mockFindMany.mockResolvedValue([
      {
        id: "hidden-record-id",
    ...
    expect(result.text).toContain("Recent record 1");
  ...
  it("bounds long context text", async () => {
    ...
    const config: ConfigMap = {
      ...
      Works: {
        ...
        DropDownOptions: Object.fromEntries(
          Array.from({ length: 200 }, (_, index) => [`work-${index}`, `Very long work option ${index} ${"z".repeat(100)}`]),
        ),
      },
    };
    ...
    expect(result.text).toContain("[context truncated to bounded extraction budget]");
  });
});
