...
export default async function DashboardIndexPage() {
	...
	const tour = (
		dbUser?.userTour && typeof dbUser.userTour === "object"
			? dbUser.userTour
			: {}
	...
	const organizationLanguage = isSuperUser
		? "en"
		: await getOrganizationLanguageByUserId(user.id);
	...
	const showAllInvoices =
		...
		Boolean(org) &&
		sites.length > 0;
	const allProjectsTitle = showAllInvoices
		...
		: organizationLanguage === "lv"
			? "Visi projekti"
			: "All projects";
	const allProjectsDescription = showAllInvoices
		? organizationLanguage === "lv"
			? "Visu projektu rēķini un apstiprināšanas statuss vienā skatā."
			: "Invoices and approval status from every project in one view."
		: organizationLanguage === "lv"
			...
			<div>
				...
				{sites.length > 0 ? (
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 auto-rows-fr">
						{showAllProjects || showAllInvoices ? (
						...
						{sites.map((item) => (
							<Card
								key={item.id}
								className="pt-0 flex flex-col h-full min-h-[380px]"
							>
								<Image
								...
								<CardFooter data-tour="dashboard/page">
									<OpenProjectButton
										...
										href={
											showAllInvoices
												? `/dashboard/invoices?project=${encodeURIComponent(item.id)}`
												: undefined
										}
										label={t.openProject}
										loadingLabel={t.openingProject}
