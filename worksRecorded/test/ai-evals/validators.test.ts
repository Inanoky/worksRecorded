...
describe("AI eval validators", () => {
  it("passes a valid context-retention answer", () => {
    const evalCase = dashboardEvalCases.find((item) => item.id === "context-retention-zone-a")!;
  ...
  it("fails unsafe save confirmations on read-only prompts", () => {
    const evalCase = dashboardEvalCases.find((item) => item.id === "read-only-site-diary")!;
    const result = validateEvalTurn(
      evalCase,
      evalCase.turns[0],
    ...
    expect(result.results.find((item) => item.name === "forbidden-claims")?.status).toBe("fail");
  });

  it("requires clarification or a limitation for ambiguous prompts", () => {
    const evalCase = dashboardEvalCases.find((item) => item.id === "ambiguous-request-clarification")!;
    const result = validateEvalTurn(
      evalCase,
      evalCase.turns[0],
    ...
    expect(result.status).toBe("fail");
    expect(result.results.find((item) => item.name === "clarification-or-limitation")?.status).toBe(
      "fail",
    );
  });
  ...
  it("does not match forbidden single words inside longer words", () => {
    ...
    expect(result.status).toBe("pass");
    expect(result.results.find((item) => item.name === "forbidden-claims")?.status).toBe("pass");
  });
  ...
  it("accepts short Latvian clarification answers as Latvian", () => {
    ...
    expect(result.status).toBe("pass");
    expect(result.results.find((item) => item.name === "language:lv")?.status).toBe("pass");
  });
  ...
  it("fails English fallback text for Latvian cases", () => {
    const evalCase = dashboardEvalCases.find((item) => item.id === "latvian-concise-error")!;
    const result = validateEvalTurn(
      evalCase,
      evalCase.turns[0],
    ...
    expect(result.status).toBe("fail");
    expect(result.results.find((item) => item.name === "language:lv")?.status).toBe("fail");
  });

  it("accepts an explicit English compatibility answer", () => {
    const evalCase = dashboardEvalCases.find((item) => item.id === "english-compatibility-read-only")!;
    const result = validateEvalTurn(
      evalCase,
      evalCase.turns[0],
    ...
    expect(result.status).toBe("pass");
    expect(result.results.find((item) => item.name === "language:en")?.status).toBe("pass");
  });
