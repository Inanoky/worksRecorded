...
function summarizeLatency(results: TurnRunResult[], slowThresholdMs: number) {
	...
	return {
		...
		slowestTurn,
		turnsOverThreshold: turns.filter(
...
async function main() {
	...
	await assertEvalEnvironment({ flow: "dashboard-chat", siteId, userId });

	const runId = createRunId();
	const agentModel =
		process.env.AI_EVAL_AGENT_MODEL || orchestratingAgentV2ModelModel;
	...
	for (const evalCase of dashboardEvalCases) {
		...
		for (const [turnIndex, turn] of evalCase.turns.entries()) {
			...
			const agentResult = await runOrchestratingAgentV2Detailed(
				turn.prompt,
				siteId,
				{
					userId,
			...
			const marker =
				deterministic.status === "fail" || judge.status === "fail"
					? "FAIL"
					: deterministic.status === "warn" || judge.status === "warn"
						? "WARN"
						: "PASS";
			const latencyLabel =
