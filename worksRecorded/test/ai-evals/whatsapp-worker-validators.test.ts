...
describe("WhatsApp worker eval validators", () => {
  const clockInCase = whatsappWorkerEvalCases.find((item) => item.id === "worker-clock-in-card")!;
  const clockOutCase = whatsappWorkerEvalCases.find((item) => item.id === "worker-clock-out")!;
  const diaryCase = whatsappWorkerEvalCases.find((item) => item.id === "worker-diary-text")!;
  const ambiguousCase = whatsappWorkerEvalCases.find((item) => item.id === "worker-ambiguous-latvian")!;
  ...
  it("passes when clock-in sends a CTA card and creates no rows", () => {
    ...
    expect(result.results.find((item) => item.name === "clock-in-card")?.status).toBe("pass");
  });
  ...
  it("fails clock-in when the card is missing", () => {
    ...
    expect(result.results.find((item) => item.name === "clock-in-card")?.status).toBe("fail");
  });
  ...
  it("fails when captured graph messages contain raw phone numbers or clock-in tokens", () => {
    ...
    expect(result.status).toBe("fail");
    expect(result.results.find((item) => item.name === "graph-message-redaction")?.status).toBe("fail");
  });
  ...
  it("passes when clock-out closes the seeded timelog", () => {
    ...
    expect(result.status).toBe("pass");
    expect(result.results.find((item) => item.name === "clock-out-closed")?.status).toBe("pass");
  });
  ...
  it("passes when work text creates a worker-owned diary record", () => {
    ...
    expect(result.results.find((item) => item.name === "worker-diary-no-user")?.status).toBe("pass");
  });
  ...
  it("fails when a worker diary eval creates a user-owned record", () => {
    ...
    expect(result.status).toBe("fail");
    expect(result.results.find((item) => item.name === "no-user-diary-record")?.status).toBe("fail");
  });
