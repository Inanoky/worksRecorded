...
function prepareWebhookPayload(args: {
	...
	if (!value || !message) {
		throw new Error(
			`Invalid WhatsApp worker eval webhook fixture for ${args.evalCase.id}.`,
		);
	}
...
function installGraphApiFetchMock(
	capturedMessages: CapturedMetaGraphMessage[],
) {
...
async function assertEvalPreconditions(args: {
	...
	if (worker.siteId !== args.siteId) {
		throw new Error(
			`Eval worker siteId must be ${args.siteId}; got ${worker.siteId ?? "null"}.`,
		);
	}
...
async function main() {
	...
	await assertEvalEnvironment({ flow: "whatsapp-worker", siteId, workerId });
	const worker = await assertEvalPreconditions({ siteId, workerId });
	const originalWorkerClockedIn = worker.isClockedIn ?? false;

	const runId = createRunId();
	const agentModel =
		process.env.AI_EVAL_AGENT_MODEL || clickInAgentForWorkersModel;
		...
		for (const evalCase of selection.selectedCases) {
			...
			try {
				...
				timelogRecordIds = Array.from(
					new Set([
						...timelogRecordIds,
						...timelogRecords.map((record) => record.id),
					]),
				);
				...
				if (preserveRecords) {
					console.log(
						`[PRESERVED] ${evalCase.id} diaryRecords=${createdDiaryRecordIds.join(",") || "none"} timelogs=${timelogRecordIds.join(",") || "none"}`,
					);
					await cleanupEvalIdentity(businessPhoneNumberId);
				} else {
	...
	const report: WhatsAppWorkerEvalReport = {
		...
		summary: {
			...
			deterministicFailures: results.reduce(
				(count, item) => count + item.deterministic.criticalFailures,
				0,
			),
			...
			deterministicWarnings: results.reduce(
				(count, item) => count + item.deterministic.warnings,
				0,
			),
	...
	if (report.summary.deterministicCriticalFailures > 0) {
		process.exitCode = 1;
	}
}
