...
function textFromGraphMessage(message: unknown) {
	...
	const interactiveBody = asString(
		asRecord(asRecord(body.interactive).body).text,
	);
	const interactiveHeader = asString(
		asRecord(asRecord(body.interactive).header).text,
	);
	return [interactiveHeader, interactiveBody, textBody]
		.filter(Boolean)
		.join("\n")
		.trim();
}
...
function containsSensitiveValue(value: unknown, key?: string): boolean {
	if (typeof value === "string") {
		...
		if ((key === "to" || key === "recipient") && /^\+?\d{8,}$/.test(value))
			return true;
		return false;
	}

	if (Array.isArray(value))
		return value.some((item) => containsSensitiveValue(item, key));
...
function hasClockInCard(item: NormalizedEvalItem) {
	return (
		stringifyArtifact(item.artifacts.graphMessages).includes("cta_url") &&
		item.caseId.includes("clock-in")
	);
}
...
function makeItem(
	...
	const validationResults = rawValidationResults.map((result) => {
		...
		return {
			...
			severity:
				...
				(asString(record.status) === "fail" ? "critical" : ""),
			message: asString(record.message),
		};
	});
	const tokenUsage = getTokenUsage(
		Object.keys(asRecord(item.tokenUsage)).length
			? item.tokenUsage
			: item.aggregateTokenUsage,
	);
	const originalContextTokens = asNumber(
		controlledMemory.originalEstimatedTokens,
	);
	const compactedContextTokens = asNumber(
		controlledMemory.compactedEstimatedTokens,
	);
	const normalizedStatus =
		normalizeStatus(deterministic.status) !== "unknown"
	...
	return {
		...
		validationResults,
		failedValidators: validationResults.filter(
			(result) => result.status === "fail" && result.severity !== "warning",
		),
		warningValidators: validationResults.filter(
			(result) => result.status === "fail" && result.severity === "warning",
		),
		contextTokens: {
			...
			saved:
				originalContextTokens !== null && compactedContextTokens !== null
		...
		artifacts: {
			...
			structuredSaveTrace,
			createdRecordIds:
				item.createdRecordIds ?? item.createdDiaryRecordIds ?? null,
...
function detectItemAnomalies(
	...
	const responseText = [item.answer, ...item.outboundMessages]
		.join("\n")
		.trim();
	...
	const failedLanguageChecks = item.validationResults.filter(
		(result) =>
			result.status === "fail" &&
			/language|latvian|english|valod/i.test(result.name + result.message),
	);
	...
	if (!responseText && item.actualModel && !expectedToolCalls) {
		anomalies.push({
			...
			code: "empty-response",
			message:
				"Model metadata exists but no answer or outbound response text was captured.",
		});
	}

	if (
		item.caseId.includes("clock-in-card") &&
		(!item.actualModel || !item.tokenTotal)
	) {
		anomalies.push({
			severity: "warning",
			code: "missing-model-metadata",
			message:
				"Clock-in card case should still retain model and token metadata.",
		});
	}
	...
	if (containsSensitiveValue(reviewPayload)) {
		anomalies.push({
			...
			message:
				"Response or artifacts appear to contain an unredacted token, phone number, or temporary Meta URL.",
		});
	}
	...
	if (
		/read-only|lasīšanas rež/i.test(item.input) &&
		/saved successfully|created successfully|saglabāts veiksmīgi/i.test(
			responseText,
		)
	) {
		anomalies.push({
			...
			message:
				"Read-only eval response appears to include a save/create confirmation.",
		});
	}
	...
	if (
		averageLatencyMs > 0 &&
		item.latencyMs > Math.max(15000, averageLatencyMs * 1.75)
	) {
		anomalies.push({
			severity: "info",
			code: "high-latency",
...
function runStatusFromSummary(
	summary: Record<string, unknown>,
	items: NormalizedEvalItem[],
): EvalStatus {
	const hasFailures = Object.entries(summary).some(
		([key, value]) =>
			/fail/i.test(key) && typeof value === "number" && value > 0,
	);
	if (
		hasFailures ||
		items.some((item) => item.status === "fail" || item.judgeStatus === "fail")
	)
		return "fail";

	const hasWarnings = Object.entries(summary).some(
		([key, value]) =>
			/warn/i.test(key) && typeof value === "number" && value > 0,
	);
	if (
		hasWarnings ||
		items.some((item) => item.status === "warn" || item.judgeStatus === "warn")
	)
		return "warn";
...
export function normalizeEvalReport(
	raw: unknown,
	...
	for (const item of items) {
		item.anomalies = detectItemAnomalies(
			item,
			flow,
			averageLatencyMs,
			duplicateAnswers,
	...
	return {
		...
		actualModels: asArray(report.actualModels).filter(
...
export async function loadEvalReports(
	resultsDir = path.join(process.cwd(), ".ai-eval-results"),
) {
	let entries: string[] = [];
	try {
