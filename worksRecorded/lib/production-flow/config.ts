...
export const FLOW_CONFIG_ADMIN_USER_IDS = [...SUPER_USER_IDS] as const;
...
export const ZTC_PRODUCTION_FLOW_CONFIG: ProductionFlowConfig = {
	...
	name: "ZTC Production",
	description:
		"ZTC-specific production flow with drawings, pauses, productivity, and element-aware additional works.",
	isDefault: false,
...
export function getProductionFlowPrimaryOrganizationId(
	...
	return (
		PRODUCTION_FLOW_CONFIGS.find(
			(config) =>
				!config.isDefault &&
				config.enabled &&
				(config.organizationIds.includes(organizationId) ||
...
export function canAccessFlowConfigAdmin(
	userId?: string | null,
	host?: string | null,
) {
	return (
		FLOW_CONFIG_ADMIN_USER_IDS.some((adminUserId) => adminUserId === userId) ||
